...
export default async function graphQLFetch(query, variables = {}) {
  try {
    ...
    if (result.errors) {
      ...
      alert(`Error while querying for data - ${error}`);
    }
    return result.data;
  } catch (e) {
    alert(`Error while querying for data: ${e.message}\n\nMake sure MongoDB is running on localhost and you have run the init.mongo.js script :)`);
    return null;
  }
}
